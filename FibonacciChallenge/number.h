...
// Thanks to the Sheafification Of G for programming this number struct.
// This is a very nice arbitrary-precision number class. Feel free to
// modify it all you would like.

// Use `number` for easiest use. No need to get into the weeds unless you
// want to.
...
namespace big
{
    ...
    {
        digit_range(const typename std::vector<T>::iterator& begin, const typename std::vector<T>::iterator& end)
    ...
    {
        const_digit_range(const typename std::vector<T>::const_iterator& begin, const typename std::vector<T>::const_iterator& end)
    ...
    std::string num_t<T>::str(bool full) const
    {
        ...
        constexpr T fullmask = 0 - static_cast<T>(1);
